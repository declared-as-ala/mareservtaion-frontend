...
export default function PanoramaEngine({
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      {!loaded && (
        <div className="absolute inset-0 flex items-center justify-center bg-zinc-900/80 z-10">
          <div className="flex flex-col items-center gap-2 text-zinc-400">
            <div className="size-8 border-2 border-zinc-500 border-t-amber-400 rounded-full animate-spin" />
      ...
      {hasScenes && loaded && (
        <div className="absolute bottom-3 left-1/2 -translate-x-1/2 z-20 flex gap-1.5 bg-black/60 backdrop-blur-sm rounded-full px-3 py-1.5 shadow-xl border border-white/10">
          {scenes!.map((scene) => {
            ...
            return (
              <button
                ...
                title={scene.name}
              >
                <span className={cn('size-1.5 rounded-full shrink-0', isActive ? 'bg-zinc-950' : 'bg-zinc-500')} />
                {scene.name}
              </button>
            );
          })}
